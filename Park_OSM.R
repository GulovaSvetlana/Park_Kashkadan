...
park_sf = read_sf('shapes/park_kashkadan.geojson')
plot(park_sf)
...
ggplot()
  ...
  geom_sf(data = park_kashkadan_all$osm_multipolygons, aes(fill = "natural"))
  theme_bw()

map_polygons = st_read ("map.osm", layer = "multipolygons", quiet = TRUE)
map_lines = st_read ("map.osm", layer = "lines", quiet = TRUE)
...
levels(map_polygons$natural) = c("Водные объекты", "Древесные насаждения", "Строения")
map_lines$highway = as.factor(map_lines$highway)
levels(map_lines$highway) = c("асфальт")

ggplot()+
geom_sf(map_polygons,
map=aes(fill=natural))+geom_sf(map_polygons,
